...
### Product-Mode Routing for Learned SLAM Evidence

The same learned SLAM artifact can be valuable for one product mode and unsafe for another. A Gaussian map may be excellent for visual inspection while still being unacceptable as localization truth; a semantic/dynamic mask may be useful for hygiene monitoring while remaining too uncertain for training export. The release contract should route each learned output by product mode rather than approving it globally.

| Product mode | ML-SLAM evidence that may contribute | Must remain separately proven | Blocker if missing |
|---|---|---|---|
| Runtime semantic map | Verified learned loop candidates, semantic-weighted localization diagnostics, accepted dynamic/static hygiene layers | Map frame, localization regression, stable-class geometry, runtime loader contract | Learned alignment changed the map without geometric residuals, covariance, MapEval, and replay evidence |
| Map-derived training export | Back-projected labels, map-scale semantic logits, learned cleaner masks, prior-assisted low-confidence queues | Pose lookup, release-state eligibility, taxonomy version, source-map acceptance, calibration/projection QA | Any training label from `dynamic_residual`, `static_transient`, `movable_static`, `fod_candidate`, `artifact`, or `unknown_review` |
| Map hygiene and change monitoring | MOS, scene flow, semantic/dynamic SLAM masks, learned permanence scores, fleet change priors | Rejected-point sidecars, reason codes, reviewer disposition, temporal evidence | Hard deletion with no removed-point digest or no distinction between dynamic motion and static-but-transient clutter |
| Digital twin and simulation transfer | Neural/Gaussian maps, semantic Gaussians, dense neural reconstructions, image-conditioned attributes | Explicit geometry export, scale source, dynamic artifact policy, texture/projection QA | Photoreal or neural surface treated as permanent geometry without conversion and source-map QA |
| Local benchmark and acceptance set | Learned disagreement maps, hard-negative tiles, prior failure regions, false-loop examples | Frozen inputs, evaluator labels, release-state masks, split hygiene, non-road domain tags | Benchmark labels generated by the same prior/model being evaluated or contaminated by release-state-ineligible points |

This routing is especially important in non-road urban districts. Parked bicycles, scooters, cafe furniture, construction barriers, campus event infrastructure, airport GSE, pallets, and stationary people may be motionless during the survey while still being invalid for a permanent runtime map or supervised training target. Learned SLAM can discover and rank those cases, but publication needs the source-map and hygiene evidence to say what the point is allowed to become.
...
6. **First-principles grounding:** [Semantic Mapping and Map Fusion](../../../10-knowledge-base/mapping/semantic-mapping-and-map-fusion-first-principles.md), [Point-Cloud Segmentation Losses and Metrics](../../../10-knowledge-base/geometry-3d/point-cloud-segmentation-losses-metrics-first-principles.md), and [Point-Cloud Representations and Voxelization](../../../10-knowledge-base/geometry-3d/point-cloud-representations-voxelization-first-principles.md) define the state, permanence, representation, loss, and evaluation primitives.
