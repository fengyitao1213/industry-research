...
>
> **Relation to existing docs**: `../overview/lidar-slam-algorithms.md` covers SLAM algorithms (KISS-ICP, LIO-SAM, FAST-LIO2). `../overview/ml-related-slam-research-scope.md` covers learned registration, learned place recognition, semantic/dynamic SLAM, neural/Gaussian maps, point-cloud removal, and the handoff into aggregated-map segmentation. `../slam-methods/lidar-map-cleaning-dynamic-removal.md` covers dynamic/static object removal, ERASOR/Removert/MapCleaner-style filtering, static-but-transient quarantine, and false-deletion risks. `../slam-methods/mapeval-point-cloud-map-quality-evaluation.md` covers direct point-cloud map-quality evaluation before semantic segmentation or map publication. `hd-map-standards-airside.md` covers format standards (AMDB, Lanelet2, OpenDRIVE). `hd-map-change-detection-maintenance.md` covers ongoing map maintenance. `neural-online-mapping-sota.md` covers real-time neural methods. `semantic-mapping-learned-priors.md` covers learned priors. This document integrates all of these into the **complete offline construction workflow** that produces the initial map for a new airport deployment.
>
...
- `../overview/ml-related-slam-research-scope.md` — learned registration, learned place recognition, semantic/dynamic SLAM, neural/Gaussian maps, point-cloud removal, and segmentation handoff scope
