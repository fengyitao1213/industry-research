...
| [SparseBEV](sparsebev.md) | Learning: ★★★★☆<br>Deployment: ★★★★☆ | `modern-core` | `prototype` | Reference fully-sparse camera 3D detector that closes the accuracy gap with dense BEV at lower compute. |
...
| [Aggregated-Map Semantic Segmentation](../overview/aggregated-map-semantic-segmentation.md) | Offline map-scale segmentation pipeline; anchors the segmentation-backbone method pages (MinkowskiNet, SPVCNN, KPConv, PointNeXt, RandLA-Net, Cylinder3D, SphereFormer, WaffleIron, SalsaNext, RangeFormer, FRNet, PTv3, FlatFormer, OctFormer, Superpoint Transformer, 2DPASS, OpenScene, Mosaic3D). |
| [3D Segmentation Class Taxonomy Design](../overview/3d-segmentation-class-taxonomy-design.md) | Class-taxonomy deep-dive companion to the aggregated-map page section 6 — stuff/things, granularity, hierarchical taxonomies, cross-dataset harmonization, open-set handling, the airside map taxonomy. |
| [3D Segmentation Training Paradigms](../overview/3d-segmentation-training-paradigms.md) | Way-you-train comparison across all 11 paradigms (supervised, multi-dataset, SSL, distillation, weak, semi-sup, DA, synthetic, PEFT, active, auto-label) with advantages/disadvantages and the layered airside recipe. |
| [Segmentation Post-Processing and Label Refinement](../overview/segmentation-post-processing-label-refinement.md) | Post-processing deep-dive — CRF, geometric smoothing, TTA, ensembling, training-free and learned panoptic extraction, calibration, multi-pass fusion, tile stitching, the recommended production stack. |
| [Large-Scale 3D Segmentation Tiling and Throughput](../overview/large-scale-3d-segmentation-tiling-and-throughput.md) | Tiling/stitching/throughput engineering for map-scale pipelines — partition strategies, halo selection, logit merging, normalization-layer seam artifacts, batched-inference orchestration, cost model. |
