...
Treat the semantic-map manifest as one member of the wider MLOps compatibility set. The model weights, taxonomy, confidence thresholds, source-map hash, calibration package, runtime map contract, telemetry schema, prompt/offboard-labeler artifacts, replay/evaluation pack, and rollback map bundle must be the exact set evaluated together. The OTA/SUMS compatibility gate for that set is `../../../50-cloud-fleet/ota/perception-slam-artifact-compatibility-matrix.md`; the MLOps scale scorecard that decides when a mismatch blocks release is `../../../50-cloud-fleet/mlops/mlops-scorecards-and-kpis-by-scale.md`.

Training exports also need an invalidation path. If the source map, pose graph, calibration, taxonomy, hygiene layer, reviewer decision, prompt/labeler artifact, or release-state mask changes, any back-projected pseudo-label batch derived from the older state becomes `suspect` until the impact graph is queried and the batch is rebuilt or waived. The protocol is `../../../50-cloud-fleet/mlops/map-derived-pseudo-label-invalidation-protocol.md`.
