...
| Utility, overhead-line, perimeter, or gantry infrastructure | GridNet-HD, DALES, Toronto-3D, WHU-Railway3D | Rare long-thin class recall, pylon/cable/insulator vs vegetation separation, LiDAR-image projection QA |
...
The checked contract artifacts are now first-class repo assets: `../../../schemas/semantic-map-manifest.schema.json` defines the release manifest shape, `../../../schemas/runtime-map-contract.schema.json` defines the runtime-loader handoff, and `../../../examples/map-contracts/` contains valid examples for both. Run `npm run map-contracts:check` before accepting a semantic-map release bundle or changing the manifest fields above.
...
| Dense urban district, campus, service road, or non-road industrial site | WHU-Urban3D, SemanticTHAB, KITTI-360, Paris-Lille-3D, Toronto-3D, GOOSE-Ex, SIP, STPLS3D, CUS3D | LiDAR-only first for repeatability; add RGB/colorized points when lighting and exposure are stable | Multi-dataset pre-train, then domain-continuation SSL; use active learning for furniture, poles, markings, gravel/grass, temporary equipment, and rare service objects | Do not inherit road-only taxonomies; validate building/ground/vegetation/furniture splits and non-road surfaces separately |
| Utility corridor, overhead/edge infrastructure, or gantry-heavy site | GridNet-HD, DALES, Toronto-3D, WHU-Railway3D | LiDAR geometry plus calibrated image projection where cameras are survey-grade; LiDAR-only ablation required | Compare SPT/sparse-conv baselines with late-fusion or image-distillation overlays; sample rare pylon/cable/insulator/pole classes aggressively | Treat GridNet-HD as a thin-class/fusion proxy only; require owned survey-map validation before adding cable or insulator classes to a release taxonomy |
