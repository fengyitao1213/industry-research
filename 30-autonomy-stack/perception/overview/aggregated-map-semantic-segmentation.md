...
|---|---|---|
| Dense urban district, depot, campus, or service road | WHU-Urban3D, KITTI-360, Paris-Lille-3D, Toronto-3D, SemanticTHAB, MLDAS, USCILab3D | MLS backbone pre-training; markings/poles/wires/driveways; high-resolution OS2-128 density stress; MLDAS for cross-LiDAR transfer and USCILab3D only after release checks |
...
| Dense urban district, campus, service road, or non-road industrial site | WHU-Urban3D, SemanticTHAB, KITTI-360, Paris-Lille-3D, Toronto-3D, GOOSE-Ex, SIP, S.MID, MLDAS, USCILab3D, Industrial3D, STPLS3D, CUS3D | LiDAR-only first for repeatability; add RGB/colorized points when lighting and exposure are stable | Multi-dataset pre-train, then domain-continuation SSL; use active learning for furniture, poles, markings, gravel/grass, temporary equipment, substation/MEP assets, and rare service objects | Do not inherit road-only taxonomies; validate building/ground/vegetation/furniture/industrial splits separately; use MLDAS for beam-count transfer and keep USCILab3D/Industrial3D behind release-maturity checks |
