...
- **MapEval/source-geometry gate** — the aggregated cloud should pass direct point-cloud map-quality checks before semantic metrics are reported. Record AC, COM, CD, MME, AWD, SCS, reference-map hash or no-reference waiver, alignment transform, tile-level failure regions, and threshold policy in the QA report. A geometrically distorted map can still receive plausible labels, so semantic mIoU is not trusted until this source-map gate passes.
