...
Use `../../50-cloud-fleet/mlops/mlops-reference-architectures-by-scale.md` when deciding which lanes should remain scripts, which should become shared services, and which promotion decisions need policy gates. Use `../../50-cloud-fleet/mlops/mlops-migration-checklist-by-scale.md` before promoting a lane from local CI to product release, fleet release, safety-critical release, or shared platform service. The CI/CD pipeline should preserve the same interfaces at every scale: dataset manifest, training run manifest, model package manifest, evaluation report, deployment manifest, monitoring event schema, and incident evidence link.
