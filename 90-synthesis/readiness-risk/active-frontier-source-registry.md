...
| `slam-ml-semantic-dynamic` | `slam-mapping` | `arxiv-cs-ro`, `icra`, `iros`, `rss-proceedings`, `ieee-xplore`, `semantic-scholar`, `github-search` | `"semantic SLAM"` OR `"dynamic-object-aware SLAM"` OR `"object-level SLAM"` OR `"multi-agent neural SLAM"` OR `"Gaussian SLAM"` OR `"neural implicit SLAM"` plus `dynamic masks`, `semantic landmarks`, `object factors`, `collaborative SLAM` | `cat:cs.RO`, ICRA/IROS/RSS, T-RO/RA-L, repo links | Find ML-related, semantic, object-level, dynamic-aware, neural/Gaussian, and collaborative SLAM methods. | weekly |
