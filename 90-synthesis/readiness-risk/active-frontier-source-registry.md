...
| `perception-map-segmentation` | `datasets-benchmarks` | `arxiv-cs-cv`, `cvf-openaccess`, `huggingface-datasets`, `zenodo`, `github-search`, `semantic-scholar` | `"large-scale 3D semantic segmentation"` OR `"point cloud semantic segmentation"` OR `"mobile laser scanning"` OR `"aggregated LiDAR map"` OR `"urban point cloud"` OR `"LiDAR image fusion"` plus `GridNet-HD`, `YUTO`, `Industrial3D`, `SemanticTHAB`, `WHU-Urban3D`, `FRACTAL`, `SUM Parts`, `OpenUrban3D`, `ECLAIR`, `S.MID`, `OpenTrench3D`, `MLDAS`, `USCILab3D`, `PTv3`, `Sonata`, `PointTransformerX`, `LIDARLearn`, `Superpoint Transformer`, `PointMamba`, `Pamba`, `Serialized Point Mamba`, `2DPASS`, `SLidR`, `ScaLR`, `DITR`, `LOSC`, `SALT`, `SAM4D` | `cat:cs.CV`, dataset card, DOI, license, project/repo links, recent date | Find aggregated-map, urban-district, utility/campus/industrial point-cloud segmentation datasets, pseudo-label tools, training architectures, and LiDAR-image transfer methods. | weekly |
...
| `slam-map-change` | `slam-mapping` | `arxiv-cs-ro`, `rss-proceedings`, `semantic-scholar`, `github-search` | `"lifelong mapping"` OR `"map change detection"` OR `"dynamic object removal"` OR `"DynPurge"` OR `"Point Restoration Network"` OR `"temporarily-static objects"` OR `"pose graph optimization"` | `cat:cs.RO`, recent date, repo links | Find map hygiene, lifelong mapping, dynamic-residual cleaning, learned map-restoration, and backend gaps. | weekly |
