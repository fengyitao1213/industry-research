...
| `slam-map-change` | `slam-mapping` | `arxiv-cs-ro`, `rss-proceedings`, `semantic-scholar`, `github-search` | `"lifelong mapping"` OR `"map change detection"` OR `"dynamic object removal"` OR `"DynPurge"` OR `"Point Restoration Network"` OR `"temporarily-static objects"` OR `"static-but-transient"` OR `"stationary people"` OR `"dynamic residual points"` OR `"visibility-map cleaning"` OR `"stale static objects"` OR `"moved object removal"` OR `"pose graph optimization"` | `cat:cs.RO`, recent date, repo links | Find map hygiene, lifelong mapping, dynamic-residual cleaning, static-but-transient removal, learned map-restoration, and backend gaps. | weekly |
