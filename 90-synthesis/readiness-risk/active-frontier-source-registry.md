...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `arxiv-cs-cv` | [arXiv cs.CV](https://arxiv.org/list/cs.CV/recent) | `preprint` | `T2` | preprint discovery | `perception`, `world-models`, `vla-vlm`, `datasets-benchmarks` | paper, arXiv ID, project page, repo | `cat:cs.CV`, recent submissions | `autonomous driving` plus occupancy, BEV, 3D detection, OOD, VLM, world model | RSS/API | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Verify paper page and artifact links before routing. | Preprint quality varies; dedupe against conference versions. |
| `arxiv-cs-ro` | [arXiv cs.RO](https://arxiv.org/list/cs.RO/recent) | `preprint` | `T2` | preprint discovery | `slam-mapping`, `validation`, `datasets-benchmarks` | paper, arXiv ID, repo, dataset page | `cat:cs.RO`, recent submissions | SLAM, odometry, mapping, pose graph, closed-loop evaluation, benchmark | RSS/API | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Verify robotics relevance and primary artifacts before routing. | Many generic robotics papers need AV relevance filters. |
| `arxiv-eess-iv` | [arXiv eess.IV](https://arxiv.org/list/eess.IV/recent) | `preprint` | `T2` | preprint discovery | `perception`, `datasets-benchmarks` | paper, arXiv ID, repo | `cat:eess.IV`, recent submissions | image/video perception, event camera, thermal, sensor fusion, driving scenes | RSS/API | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Verify driving or robotic perception fit before routing. | Medical and generic image papers require negative filtering. |
| `arxiv-eess-sy` | [arXiv eess.SY](https://arxiv.org/list/eess.SY/recent) | `preprint` | `T2` | preprint discovery | `validation`, `slam-mapping` | paper, arXiv ID, repo | `cat:eess.SY`, recent submissions | control, safety, runtime verification, uncertainty, fault injection, localization | RSS/API | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Verify autonomy or safety-case relevance before routing. | Broad systems/control category can be noisy. |
| `openreview-venues` | [OpenReview venues](https://openreview.net/venues) | `venue` | `T1` | open-review venue discovery | `world-models`, `vla-vlm`, `perception`, `validation` | paper, OpenReview ID, decision, reviews | venue invitation, title, abstract, decision | ICLR, ICML, NeurIPS, CoRL with driving, embodied, world model, VLA, robustness | API/manual | weekly | 2026-05-12 | 2026-05-19 | `stale` | `medium` | Verify venue decision and official paper page before routing. | Venue fields differ and API versions vary. |
...
| `semantic-scholar` | [Semantic Scholar](https://www.semanticscholar.org/) | `publisher-index` | `T3` | metadata aggregator | `perception`, `slam-mapping`, `world-models`, `vla-vlm`, `validation` | paper metadata, DOI, arXiv ID, citations | query, field of study, date | exact method names, citation trails, related work, code links | API/manual | weekly | 2026-05-12 | 2026-05-19 | `stale` | `medium` | Use only to discover or cross-check primary sources. | Aggregator metadata can lag or merge records incorrectly. |
| `openalex` | [OpenAlex](https://openalex.org/) | `publisher-index` | `T3` | metadata aggregator | `perception`, `slam-mapping`, `world-models`, `validation` | DOI, venue, authors, concepts | works search, concept, date | topic discovery, publication date checks, DOI lookup | API/manual | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Use only to discover or cross-check primary sources. | Concept tags are broad and require manual validation. |
...
| `github-search` | [GitHub Search](https://github.com/search) | `code` | `T2` | code artifact discovery | `perception`, `slam-mapping`, `world-models`, `vla-vlm`, `datasets-benchmarks` | repo, release, license, README, issues | topic, language, stars, pushed date | autonomous-driving occupancy SLAM VLM dataset benchmark stars pushed | API/manual | weekly | 2026-05-12 | 2026-05-19 | `stale` | `high` | Verify official repo link from paper/project page before routing. | Stars are weak evidence; do not clone arbitrary repos in CI. |
| `huggingface-papers` | [Hugging Face Papers](https://huggingface.co/papers) | `model` | `T3` | model and paper discovery | `world-models`, `vla-vlm`, `perception`, `datasets-benchmarks` | paper, model card, dataset card, license | papers, models, datasets, date | driving VLM, robot foundation model, world model, occupancy, dataset | manual/API | weekly | 2026-05-12 | 2026-05-19 | `stale` | `medium` | Verify canonical paper and license before routing. | Trending pages are discovery signals only. |
| `huggingface-datasets` | [Hugging Face Datasets](https://huggingface.co/datasets) | `dataset` | `T2` | dataset artifact discovery | `datasets-benchmarks`, `perception`, `world-models`, `vla-vlm` | dataset card, files, license, dataset script, paper link | task, modality, organization, license, modified date | point cloud semantic segmentation, LiDAR dataset, occupancy dataset, map segmentation, driving dataset | API/manual | weekly | 2026-05-24 | 2026-05-31 | `active` | `medium` | Verify the dataset card, owner, linked paper/project page, files, access constraints, and license before routing. | Viewer support is weak for LAS/LAZ/PCD datasets; cards can wrap external storage or auto-converted formats that are not training-ready. |
| `zenodo` | [Zenodo](https://zenodo.org/) | `dataset` | `T2` | dataset DOI source | `datasets-benchmarks`, `perception`, `slam-mapping`, `validation` | DOI, dataset record, files, version, license, related publication | community, keyword, publication date, DOI | LiDAR semantic segmentation, point cloud benchmark, robotics dataset, adverse weather dataset | manual/API | monthly | 2026-05-24 | 2026-06-24 | `active` | `medium` | Verify dataset version, file availability, license, creator identity, and linked paper before routing. | Records can be replaced by newer versions; cite the exact record/version used. |
...
| `perception-map-segmentation` | `datasets-benchmarks` | `arxiv-cs-cv`, `cvf-openaccess`, `huggingface-datasets`, `zenodo`, `github-search`, `semantic-scholar` | `"large-scale 3D semantic segmentation"` OR `"point cloud semantic segmentation"` OR `"mobile laser scanning"` OR `"aggregated LiDAR map"` OR `"urban point cloud"` plus `GridNet-HD`, `YUTO`, `Industrial3D`, `SemanticTHAB`, `WHU-Urban3D`, `FRACTAL`, `SUM Parts`, `OpenUrban3D`, `ECLAIR`, `S.MID`, `OpenTrench3D`, `MLDAS`, `USCILab3D`, `Point Cloud City`, `Open3D-ML PCC`, `City-Facade`, `ZAHA`, `LoFG`, `TUM2TWIN`, `NIST WF-PST` | `cat:cs.CV`, dataset card, DOI, license, project/repo links, recent date | Find aggregated-map, urban-district, utility/campus/industrial, managed-building, and facade point-cloud segmentation datasets and benchmark/proxy releases. | weekly |
| `perception-map-segmentation-architectures` | `perception` | `arxiv-cs-cv`, `cvf-openaccess`, `openreview-venues`, `github-search`, `semantic-scholar` | `"aggregated LiDAR map"` OR `"large-scale point cloud segmentation"` OR `"LiDAR image fusion"` plus `PTv3`, `Sonata`, `PointTransformerX`, `LIDARLearn`, `Superpoint Transformer`, `PointMamba`, `Pamba`, `Serialized Point Mamba`, `2DPASS`, `SLidR`, `ScaLR`, `DITR`, `LOSC`, `SALT`, `SAM4D` | `cat:cs.CV`, project/repo links, benchmark tables, recent date | Find training architectures, foundation/pseudo-label transfer, and LiDAR-image distillation methods for aggregated-map semantic segmentation. | weekly |
...
| `slam-lio-radar` | `slam-mapping` | `arxiv-cs-ro`, `icra`, `iros`, `rss-proceedings`, `ieee-xplore` | `"LiDAR-inertial odometry"` OR `"radar odometry"` OR `"visual-inertial SLAM"` OR `"georeferenced point cloud mapping"` OR `"map priors"` plus benchmark, `OpenLiDARMap`, `FlexCloud` | `cat:cs.RO`, ICRA/IROS/RSS, T-RO/RA-L | Find SLAM, odometry, localization, and georeferenced map-construction methods. | weekly |
| `slam-ml-semantic-dynamic` | `slam-mapping` | `arxiv-cs-ro`, `icra`, `iros`, `rss-proceedings`, `ieee-xplore`, `semantic-scholar`, `github-search` | `"semantic SLAM"` OR `"dynamic-object-aware SLAM"` OR `"object-level SLAM"` OR `"multi-agent neural SLAM"` OR `"Gaussian SLAM"` OR `"neural implicit SLAM"` OR `"semantic aggregated map"` plus `dynamic masks`, `semantic landmarks`, `object factors`, `collaborative SLAM`, `point cloud removal`, `static-but-transient` | `cat:cs.RO`, ICRA/IROS/RSS, T-RO/RA-L, repo links | Find ML-related, semantic, object-level, dynamic-aware, neural/Gaussian, and collaborative SLAM methods that improve map substrates, point-cloud removal, and aggregated-map segmentation handoffs. | weekly |
| `slam-map-change` | `slam-mapping` | `arxiv-cs-ro`, `rss-proceedings`, `semantic-scholar`, `github-search` | `"lifelong mapping"` OR `"map change detection"` OR `"dynamic object removal"` OR `"multi-session point cloud map merging"` OR `"point cloud map quality"` OR `"LAMM"` OR `"LEMON-Mapping"` OR `"MapEval"` OR `"MS-Mapping"` OR `"DynPurge"` OR `"Point Restoration Network"` OR `"temporarily-static objects"` OR `"static-but-transient"` OR `"stationary people"` OR `"dynamic residual points"` OR `"visibility-map cleaning"` OR `"stale static objects"` OR `"moved object removal"` OR `"pose graph optimization"` | `cat:cs.RO`, recent date, repo links | Find map hygiene, lifelong mapping, dynamic-residual cleaning, static-but-transient removal, learned map-restoration, multi-session map merging, map-quality evaluation, and backend gaps. | weekly |
| `semantic-map-contracts` | `validation` | `github-search`, `semantic-scholar`, `ieee-xplore`, `asam-openx`, `openalex` | `"semantic map manifest"` OR `"runtime map contract"` OR `"HD map schema"` OR `"map compatibility hash"` OR `"semantic layer schema"` OR `"prior input provenance"` OR `"map publication gate"` | standards pages, schema docs, repo links, recent date | Find semantic-map schema, provenance, compatibility, and publication-gate contract patterns. | monthly |
