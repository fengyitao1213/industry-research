...
| `perception-map-segmentation` | `datasets-benchmarks` | `arxiv-cs-cv`, `cvf-openaccess`, `huggingface-datasets`, `zenodo`, `github-search`, `semantic-scholar` | `"large-scale 3D semantic segmentation"` OR `"point cloud semantic segmentation"` OR `"mobile laser scanning"` OR `"aggregated LiDAR map"` OR `"urban point cloud"` plus `GridNet-HD`, `YUTO`, `Industrial3D`, `SemanticTHAB`, `WHU-Urban3D`, `FRACTAL`, `SUM Parts`, `OpenUrban3D`, `ECLAIR`, `S.MID`, `OpenTrench3D`, `MLDAS`, `USCILab3D` | `cat:cs.CV`, dataset card, DOI, license, project/repo links, recent date | Find aggregated-map, urban-district, utility/campus/industrial point-cloud segmentation datasets and benchmark/proxy releases. | weekly |
